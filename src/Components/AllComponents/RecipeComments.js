...
const getRecipeById = async (recipeId) => {
  ...
  const handleDelete = async(commentId,text)=>{
    ...
    if(token){
      ...
      let response = await fetch(`${API_URL}/http://localhost:4500/deletecomment/${userId}/${recipeId}/${commentId}`,{
        method:"DELETE",
        headers:{
          'Content-Type': 'application/json',
          cookie: 'Token ' + token,
  ...
  const handleCommentSubmit = async (event) => {
    ...
    if (token) {
      ...
      const response = await fetch(`${API_URL}/http://localhost:4500/comment/${userId}/${recipeId}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          cookie: 'Token ' + token,
