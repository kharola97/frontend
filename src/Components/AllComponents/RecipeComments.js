...
const getRecipeById = async (recipeId) => {
  ...
  const handleDelete = async(commentId,text)=>{
    ...
    if(token){
      ...
      let response = await fetch(`https://myapp-j3dc.onrender.com/deletecomment/${userId}/${recipeId}/${commentId}`,{
        method:"DELETE",
        headers:{
          'Content-Type': 'application/json',
          cookie: 'Token ' + token,
  ...
  const handleCommentSubmit = async (event) => {
    ...
    if (token) {
      ...
      const response = await fetch(`https://myapp-j3dc.onrender.com/comment/${userId}/${recipeId}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          cookie: 'Token ' + token,
